...
const webConfigPath = resolve(serverDir, "web", "config.js");
...
if (useCompose) {
  ...
  if (deployWebUi) {
    ...
    // Gitignored, per-deployment (see web/config.example.js) — reuses
    // the API's own public base URL rather than asking a second time,
    // since that's exactly what the web UI needs to call. Writing it
    // here (not a tracked file) is what lets a future `git pull` never
    // conflict with a deployment's own value again.
    writeFileSync(
      webConfigPath,
      `// Generated by scripts/setup.mjs from PUBLIC_BASE_URL — see\n` +
      `// web/config.example.js for what this does and how to set it by hand.\n` +
      `export const API_BASE_URL = "${publicBaseUrl}";\n`,
    );
    console.log(`Wrote ${webConfigPath}`);
